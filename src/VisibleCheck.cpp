...
static bool CheckSpottedByMask(uintptr_t pawn, int localIndex) {
    ...
    int maskOff = g_fldSpottedByMask.valid ? g_fldSpottedByMask.value : schemas::EntitySpottedState_t::m_bSpottedByMask;
    uintptr_t stateAddr = pawn + OffEntitySpottedState();
    uint32_t mask[2];
    mask[0] = mem.Read<uint32_t>(stateAddr + maskOff);
    mask[1] = mem.Read<uint32_t>(stateAddr + maskOff + 4);
