...
int g_entityIdentityStride = 112;
int g_boneJointStride = 32;
ScannedOffset g_fldSpottedByMask;

static int32_t FindClassSize(uintptr_t typeScope, const char* className);
...
static int32_t FindClassSize(uintptr_t typeScope, const char* className) {
    uintptr_t binding = FindClassBinding(typeScope, className);
    if (!binding) return 0;
    return mem.Read<int32_t>(binding + 0x20);
}
...
static void RefreshSchemaOffsets() {
    ...
    if (int32_t size = FindClassSize(typeScope, "CEntityIdentity"))
        g_entityIdentityStride = size;

    if (int32_t size = FindClassSize(typeScope, "CJointData"))
        g_boneJointStride = size;

    if (uintptr_t spottedType = FindClassBinding(typeScope, "EntitySpottedState_t")) {
        if (FindSchemaField(spottedType, "m_bSpottedByMask", offset))
            g_fldSpottedByMask = { offset, true };
    }
}
